 ...
 * @requiredField query
 */
declare function typedQueryOrderPaymentRequests(query: OrderPaymentRequestQuery): Promise<NonNullablePaths<QueryOrderPaymentRequestsResponse, `orderPaymentRequests` | `orderPaymentRequests.${number}.source.externalId` | `orderPaymentRequests.${number}.status` | `orderPaymentRequests.${number}.orderId` | `orderPaymentRequests.${number}.amount.amount` | `orderPaymentRequests.${number}.amount.formattedAmount` | `orderPaymentRequests.${number}.currency` | `orderPaymentRequests.${number}.title`, 5>>;
interface OrderPaymentRequestQuerySpec extends QuerySpec {
    paging: 'cursor';
    wql: [
        {
            fields: [
...
type CommonQueryWithEntityContext = Query<OrderPaymentRequest, OrderPaymentRequestQuerySpec>;
type OrderPaymentRequestQuery = {
    /**
    Cursor token pointing to a page of results. Not used in the first request. Following requests use the cursor token and not `filter` or `sort`.
    */
    cursorPaging?: {
