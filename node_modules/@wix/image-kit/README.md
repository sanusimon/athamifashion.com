...
| allowAnimatedTransform| true, false, undefined | undefined | boolean |
| allowGIFFullTransformation| true, false, undefined | undefined | boolean |

**Note on animated images:**
- `allowAnimatedTransform`: Controls whether animated images (WebP, AVIF, GIF) can be transformed. Used primarily in placeholder flow to enable poster generation (first frame). Defaults to `true` in placeholder flow unless explicitly set to `false`.
- `allowGIFFullTransformation`: Controls whether GIFs can be fully transformed (with animation preserved) in non-placeholder flows. When `true`, uses `anm_-1` flag to preserve full animation. Only applies to GIF images in non-placeholder flows.
